...
-- Update anomaly
-- Imagine a situation where the price of the "Shirt" (ProductID:17 ) needs to be updated from ₹50,000 to ₹52,000. Here's the potential update anomaly:
...
-- If you have existing orders for the "Shirt" placed at the old price (₹50,000), these orders won't be automatically updated to reflect the new price (₹52,000). 
-- This inconsistency can lead to issues like incorrect revenue calculations or customer disputes.

-- DELETE ANOMALY
-- Imagine a situation where a specific product, the "Pant" (ProductID: 04), becomes discontinued and needs to be removed from the database. Here's how a delete anomaly could occur:

-- You delete the "Pant" record from the Products table:
...
-- The problem arises if there are existing orders for the "Pant" in the Orders table. Deleting the product from Products would leave those orders referencing a non-existent product.

-- INSERTION ANOMALY

-- Imagine you want to create a new order for a customer (CustomerID: 11) who hasn't been added to the Customers table yet. Here's how an insertion anomaly could occur:


INSERT INTO Orders (OrderID, CustomerID, ProductID, Quantity)
VALUES (1111, 11, 17, 1704);  -- Assuming ProductID 2 is 'Realme Mobile'

-- The insertion fails because the CustomerID (11) doesn't exist in the Customers table.
...
1111	102				17		3000				Blue
1100	104				04		1999			    Black
1704	105				71		1704				White
1444	101				40		4017				Silver
